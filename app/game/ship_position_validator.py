...
class ShipPositionValidator(object):
    ...
    def can_ship_be_put_there(self):
        to_return = True
    ...
    def vertical_check_top_boundary(self):
        for i in range(self.size + 1):
            if self.board[self.start_row + i][self.col - 1] != '*' or self.board[self.start_row + i][self.col] != '*' or \
                            self.board[self.start_row + i][self.col + 1] != '*':
                return False
        return True

    def vertical_check_bottom_boundary(self):
        for i in range(self.size + 1):
            if self.board[self.end_row - i][self.col - 1] != '*' or self.board[self.end_row - i][self.col] != '*' or \
                            self.board[self.end_row - i][self.col + 1] != '*':
                return False
        return True
